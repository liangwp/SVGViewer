function lwpSVGViewer(svg) // entry point. Call this function from window.onload, giving the target svg.
{
    ...
    slider1.addEventListener("input", slider1_move);
    slider2.addEventListener("input", function() {
        slider2.label.innerHTML = "zoom level 2: " + slider2.value;
    });
    ...
    function autopan(tx, ty) {
        ...
        function animate() {
            ...
            var interpolated = anim_interpolation_fn({
                x: autopan_delta_svg_x,
                y: autopan_delta_svg_y
            });
    ...
    function slider1_move(e) { // oops.... I broke it...
        svg.zoom = slider1.value;
        slider1.label.innerHTML = "zoom level 1: " + slider1.value;

        console.log("min, max, value: " + slider1.min + ", " + slider1.max + ", " + slider1.value);

        var element_x = svg.getBoundingClientRect().left;
        var element_y = svg.getBoundingClientRect().top;
        var element_centre_x = Math.round(svg.getBoundingClientRect().width / 2) + element_x;
        var element_centre_y = Math.round(svg.getBoundingClientRect().width / 2) + element_y;

        var target_x = svg.viewBox.baseVal.x + Math.round(element_centre_x / svg.zoom);
        var target_y = svg.viewBox.baseVal.y + Math.round(element_centre_y / svg.zoom);
        // target_x and target_y must be the svg coordinates at the element centre
        svg.viewBox.baseVal.x = target_x + (svg.viewBox.baseVal.x - target_x / 2) * svg.zoom;
        svg.viewBox.baseVal.y = target_y + (svg.viewBox.baseVal.y - target_y / 2) * svg.zoom;

        svg.viewBox.baseVal.width = Math.round(svg.full_width / svg.zoom);
        svg.viewBox.baseVal.height = Math.round(svg.full_height / svg.zoom);
    ...
    function toSVGCoords(client_coords_obj) {
        ...
        return {
            x: viewbox_x,
            y: viewbox_y
        }; // viewbox_x and viewbox_y are the coordinates in scaled svg-space
    }
